...
- (void)viewDidLoad
{
    ...
    _tableView=[[UITableView alloc] initWithFrame:CGRectMake(0, 50, self.view.frame.size.width, 360)];
    ...
    _speekBtn=[[UIButton alloc] initWithFrame:CGRectMake(0, 440, self.view.frame.size.width, 40)];
    [_speekBtn setTitle:@"按住说话" forState:0];
    [_speekBtn setBackgroundColor:[UIColor redColor]];
    ...
    //录音按钮添加手势操作
    [_speekBtn addGestureRecognizer:guesture];
}
...
- (void)handSpeakBtnPressed:(UILongPressGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        [_speekBtn setTitle:@"松开结束" forState:UIControlStateNormal];
